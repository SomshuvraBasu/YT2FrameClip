...
def create_video_clip(video_path, output_folder, start_time, end_time):
    try:
        ...
        output_file_path = os.path.join(output_folder, "video_clip.webm")

        # Run ffmpeg command to extract video clip
        command = [
            "ffmpeg",
            "-i", video_path,
            "-ss", str(start_time),
        ...
        print(f"Video clip created from {start_time} to {end_time} and saved to '{output_file_path}'")
    except Exception as e:
        print(f"An error occurred: {str(e)}")
...
if __name__ == "__main__":
    ...
    if video_file_path:
        try:
            ...
            output_folder = os.path.join(os.path.dirname(video_file_path), f"{video_name}_video_clip_from_{start_time_str}_to_{end_time_str}")

            extract_frames(video_file_path, output_folder, start_time, end_time)
            create_video_clip(video_file_path, output_folder, start_time, end_time)
        except Exception as e:
            print(f"An error occurred: {str(e)}")
